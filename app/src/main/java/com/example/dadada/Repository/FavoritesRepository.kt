...
class FavoritesRepository(
    ...
    fun getFavorites(): List<FilmItemModel> {
    ...
    fun isFavorite(item: FilmItemModel): Boolean {
        val targetKey = filmKey(item)
        return getFavorites().any { filmKey(it) == targetKey }
    }

    fun addFavorite(item: FilmItemModel): Boolean {
        val favorites = getFavorites().toMutableList()
        val targetKey = filmKey(item)
        if (favorites.any { filmKey(it) == targetKey }) {
            return false
        }
        favorites.add(item)
    ...
    fun removeFavorite(item: FilmItemModel): Boolean {
        val favorites = getFavorites().toMutableList()
        val targetKey = filmKey(item)
        val removed = favorites.removeAll { filmKey(it) == targetKey }
        if (removed) {
            saveFavorites(favorites)
        }
        return removed
    ...
    private fun saveFavorites(items: List<FilmItemModel>) {
    ...
    companion object {
        private const val LEGACY_PREFS_NAME = "favorites_prefs"
        private const val KEY_ITEMS = "favorite_items"
